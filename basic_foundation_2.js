...
// changes all positive numbers in the array to the 
// string "big".  Example: makeItBig([-1,3,5,-5]) 
// returns that same array, changed to 
// [-1, "big", "big", -5].
...
// in an array of numbers.  The function should print the 
// lowest value in the array, and return the highest value 
// in the array.
...
// in an array of numbers.  The function should print the 
// second-to-last value in the array, and return the first 
// odd value in the array.

function printSLreturnFO(arr){
    for (var i = 0; i < arr.length; i++){
        if (arr[i] % 2 != 0){
...
console.log(printSLreturnFO([2,2,2]));

// Double Vision - Given an array (similar to saying 'takes 
// in an array'), create a function that returns a new array 
...
// positive values found in the array.  Example, 
// countPositives([-1,1,1,1]) changes the original array 
...
function countPositives(arr){
    ...
    arr[arr.length - 1] = numPositives;
    return arr;
}

console.log(countPositives([-1,1,1,1]));

// Evens and Odds - Create a function that accepts an 
// array.  Every time that array has three odd values in 
// a row, print "That's odd!".  Every time the array has 
...
// index is odd (arr[1], arr[3], arr[5], etc).  Afterward, 
...
// Previous Lengths - You are passed an array (similar 
// to saying 'takes in an array' or 'given an array') 
// containing strings.  Working within that same array, 
// replace each string with a number - the length of the 
// string at the previous array index - and return the 
// array.  For example, previousLengths(["hello", "dojo", 
// "awesome"]) should return ["hello", 5, 4]. Hint: Can 
...
// reverses its values, in-place.  Example: 
// reverse([3,1,6,4,2]) returns the same array, but now 
// contains values reversed like so... [2,4,6,1,3].  Do 
// this without creating an empty temporary array.  
...
// Outlook: Negative - Given an array, create and return 
// a new one containing all the values of the original array, 
// but make them all negative (not simply multiplied by -1). 
...
// to "food".  If no array values are "food", then print 
// "I'm hungry" once.
...
console.log(alwaysHungry(["air", "aru", "food"]));

// Swap Toward the Center - Given an array, swap the first 
...
// swapTowardCenter([1,2,3,4,5,6]) turns the array into 
...
// and return the changed array arr.  For example, 
